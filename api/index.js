...
// ── CONFIGURATION ─────────────────────────────────────────────
// Uses env vars (set in Vercel dashboard) with hardcoded fallback for dev
const SUPABASE_URL = process.env.SUPABASE_URL  || "https://yksrbucnboocbeqspbfo.supabase.co";
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inlrc3JidWNuYm9vY2JlcXNwYmZvIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NTE5MDkyNSwiZXhwIjoyMDkwNzY2OTI1fQ.c_myWAzQz4jzZTM843lXCWBTIyFh3YEzhKZl7sbX3qs";
...
async function route(action, data) {
  switch (action) {
    ...
    case 'bulkAddWorkers':              return bulkAddWorkers(data);
    // Meters
    ...
    case 'bulkAddMeters':               return bulkAddMeters(data);
    // Readings
    ...
    case 'bulkAddReadings':             return bulkAddReadings(data);
    // Master meter
    ...
    case 'resolveAnomaly':              return resolveAnomaly(data);
...
async function login({ username, password }) {
  ...
  const rows = await sb(
    `users?username=eq.${encodeURIComponent(username)}&password=eq.${hashed}&status=eq.active&select=id,username,role,full_name,email`
  );
...
async function getUsers() {
  ...
  return {
    success: true,
    data: rows.map(u => ({
      UserID: u.id, Username: u.username, Role: u.role, FullName: u.full_name,
...
async function addUser({ username, password, role, fullName, email, phone }) {
  const row = await sb('users', 'POST', {
    username, password: hashPass(password), role,
    full_name: fullName || '', email: email || '', phone: phone || '', status: 'active'
  });
...
async function getWorkers({ status } = {}) {
  ...
  return {
    success: true,
    data: rows.map(w => ({
      WorkerID: w.id, EmployeeCode: w.employee_code, Name: w.name,
      Department: w.department, Designation: w.designation,
      Phone: w.phone, Email: w.email, JoiningDate: w.joining_date,
      Status: w.status, Address: w.address, MeterNumber: w.meter_number,
...
// BULK WORKERS — accepts { rows: [...] }
async function bulkAddWorkers({ rows = [] }) {
  let successCount = 0;
  const errors = [];
  for (let i = 0; i < rows.length; i++) {
    const d = rows[i];
    try {
      if (!d.Name && !d.name) { errors.push(`Row ${i + 2}: Name is required`); continue; }
      const res = await addWorker(d);
      if (res.success) successCount++;
      else errors.push(`Row ${i + 2} (${d.Name || d.name}): ${res.message}`);
    } catch (e) {
      errors.push(`Row ${i + 2}: ${e.message}`);
    }
  }
  return { success: true, successCount, errorCount: errors.length, errors, message: `${successCount} workers added, ${errors.length} failed` };
}
...
async function getMeters({ status } = {}) {
  ...
  return {
    success: true,
    data: rows.map(m => ({
      MeterID: m.id, MeterNumber: m.meter_number, Location: m.location,
      WorkerID: m.worker_id, WorkerName: m.worker_name,
      InstallDate: m.install_date, Status: m.status,
      MaxLoad: m.max_load, Phase: m.phase, Remarks: m.remarks,
...
// BULK METERS — accepts { rows: [...] }
async function bulkAddMeters({ rows = [] }) {
  let successCount = 0;
  const errors = [];
  for (let i = 0; i < rows.length; i++) {
    const d = rows[i];
    try {
      if (!d.MeterNumber && !d.meter_number) { errors.push(`Row ${i + 2}: MeterNumber is required`); continue; }
      const res = await addMeter(d);
      if (res.success) successCount++;
      else errors.push(`Row ${i + 2} (${d.MeterNumber || d.meter_number}): ${res.message}`);
    } catch (e) {
      errors.push(`Row ${i + 2}: ${e.message}`);
    }
  }
  return { success: true, successCount, errorCount: errors.length, errors, message: `${successCount} meters added, ${errors.length} failed` };
}
...
async function addReading(d) {
  ...
  const consumption = parseFloat(d.ReadingValue || d.reading_value) -
                      parseFloat(d.PreviousReading || d.previous_reading || 0);

  // Non-blocking anomaly check
  checkAnomaly(
    d.MeterID || d.meter_id, d.MeterNumber || d.meter_number,
    d.WorkerID || d.worker_id, d.WorkerName || d.worker_name,
...
async function updateReading(d) {
  ...
  const readingDate = d.ReadingDate || d.reading_date;
  const dt          = new Date(readingDate);
  ...
  await sb(`meter_readings?id=eq.${id}`, 'PATCH', {
    ...
    previous_reading: parseFloat(d.PreviousReading || d.previous_reading || 0),
    reading_date:     readingDate,
    month:            dt.getMonth() + 1,   // FIX: update month/year when date changes
    year:             dt.getFullYear(),
...
// BULK READINGS — accepts { rows: [...] }
// MeterNumber is resolved to MeterID via DB lookup
async function bulkAddReadings({ rows = [] }) {
  let successCount = 0;
  const errors = [];

  // Batch-load all meters once to avoid N+1 lookups
  const allMeters = await sb('meters?select=id,meter_number,worker_id,worker_name');
  const meterMap  = Object.fromEntries(allMeters.map(m => [m.meter_number, m]));

  for (let i = 0; i < rows.length; i++) {
    const d = rows[i];
    try {
      const meterNum = d.MeterNumber || d.meter_number || '';
      if (!meterNum) { errors.push(`Row ${i + 2}: MeterNumber is required`); continue; }
      const meter = meterMap[meterNum];
      if (!meter) { errors.push(`Row ${i + 2}: Meter "${meterNum}" not found`); continue; }

      const readingVal = parseFloat(d.ReadingValue || d.reading_value || 0);
      if (!readingVal) { errors.push(`Row ${i + 2}: Invalid ReadingValue`); continue; }

      const enriched = {
        MeterID:         meter.id,
        MeterNumber:     meter.meter_number,
        WorkerID:        meter.worker_id,
        WorkerName:      meter.worker_name,
        ReadingDate:     d.ReadingDate    || d.reading_date  || new Date().toISOString().split('T')[0],
        ReadingValue:    readingVal,
        PreviousReading: parseFloat(d.PreviousReading || d.previous_reading || 0),
        EnteredBy:       d.EnteredBy || d.entered_by || 'bulk-upload'
      };

      const res = await addReading(enriched);
      if (res.success) successCount++;
      else errors.push(`Row ${i + 2} (${meterNum}): ${res.message}`);
    } catch (e) {
      errors.push(`Row ${i + 2}: ${e.message}`);
    }
  }

  return { success: true, successCount, errorCount: errors.length, errors, message: `${successCount} readings added, ${errors.length} failed` };
}
...
// ── ANOMALIES ─────────────────────────────────────────────────
async function getAnomalies({ type, year, status } = {}) {
...
async function resolveAnomaly({ id }) {
  await sb(`anomalies?id=eq.${id}`, 'PATCH', { status: 'resolved' });
...
async function checkAnomaly(meterId, meterNumber, workerId, workerName, consumption, month, year) {
  try {
    ...
    await sb(
      `meter_readings?meter_id=eq.${meterId}&month=eq.${month}&year=eq.${year}`,
      'PATCH',
      { is_anomaly: true, anomaly_reason: `${type} — ${deviation.toFixed(1)}% vs avg ${avg.toFixed(0)}` }
    );
...
async function getInactiveWithConsumption() {
  ...
  // FIX: UUIDs in PostgREST `in` filter must NOT be quoted
...
async function getReport({ reportType, filters = {} }) {
  ...
  if (reportType === 'department_wise') {
    let rq = 'meter_readings?select=consumption,worker_id,month,year';
    if (filters.year) rq += `&year=eq.${filters.year}`;
    const readings   = await sb(rq);
    const allWorkers = await sb('workers?select=id,name,department');
    ...
    readings.forEach(r => {
      ...
      if (!deptData[dept]) deptData[dept] = { Department: dept, TotalConsumption: 0, ReadingCount: 0, WorkerSet: new Set() };
      deptData[dept].TotalConsumption += parseFloat(r.consumption || 0);
      deptData[dept].ReadingCount++;
      if (r.worker_id) deptData[dept].WorkerSet.add(r.worker_id);
    });

    const result = Object.values(deptData).map(d => ({
      Department:      d.Department,
      TotalConsumption: d.TotalConsumption.toFixed(2),
      ReadingCount:    d.ReadingCount,
      WorkerCount:     d.WorkerSet.size,
      AvgPerWorker:    d.WorkerSet.size > 0 ? (d.TotalConsumption / d.WorkerSet.size).toFixed(2) : '0.00'
    })).sort((a, b) => parseFloat(b.TotalConsumption) - parseFloat(a.TotalConsumption));

    return { success: true, data: result, count: result.length };
  }

  if (reportType === 'anomalies')          return getAnomalies(filters);
  if (reportType === 'inactive_consuming') return getInactiveWithConsumption();
...
async function sendAlertEmail({ to, subject, body }) {
  if (RESEND_KEY) {
    try {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        body:    JSON.stringify({ from: 'EMMS <alerts@ballyjutemill.com>', to: to.split(','), subject, text: body })
      });
  ...
  return { success: false, message: 'No email provider configured. Add RESEND_API_KEY in Vercel env vars.' };
}
